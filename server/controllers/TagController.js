...
exports.createTag = async (req,res) => {
    try {
        ...
        console.log('Request name:', tagName);
        console.log('Guest ID in CreateTag:', req.guestId);
        if(!tagName) {
            return res.status(400).json({error:'Tag name is required'})
        }

        const formatUser = isValidObjectId(user) ? new ObjectId(user) : null
...
exports.removeTag = async (req,res) => {
    try{
        const { id } = req.params;
        const formatUser = req.user && isValidObjectId(req.user) ? new Types.ObjectId(req.user.id) : null;
        const formatId = req.user && isValidObjectId(id) ? new Types.ObjectId(id) : null;
        ...
        return res.status(200).json({message:'Removed tag successful',removedTag})
    } catch(error){
