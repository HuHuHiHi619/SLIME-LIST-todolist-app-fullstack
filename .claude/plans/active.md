...
## Decisions
- **Navigation:** Sidebar is killed. Navbar carries: Dashboard | Tasks | Focus (Pomodoro). Pet nav item added in step 4 when Pet page is built. Settings + Instructions + Logout move into avatar dropdown menu (AvatarMenu.jsx).
...
1. **Shell:** ✅ DONE — branch `phase3/navbar-redesign`. Grid navbar (logo | links | avatar), sidebar killed, uiSlice pruned (removed isSidebarPinned/activeMenu/isHover), AvatarMenu.jsx replaces NotificationForm. Nav: Dashboard → `/`, Tasks → `/all-tasks`, Focus → `/pomodoro`.
2. **Dashboard:** ✅ DONE — branch `phase3/dashboard-redesign`. 4 panels built. 5 bugs fixed. See `.claude/history/2026-06-18-dashboard-handoff.md`.
